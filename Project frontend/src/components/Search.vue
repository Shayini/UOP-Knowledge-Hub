    ...
    <div class="search">
        ...
        <div>
            <form @submit.prevent="search()">
                <input id="textbox1" type="text"  list="dialog" autocomplete="off" name="search-text" v-on:keyup="display()" placeholder="Search..." v-model.trim="input" >
        ...
        <div class="SearchResult">
            <table class="table3" v-if="output && output.length">
                <tr v-for="element in output" :key="element.id">
                    <td><a :href="element.file_path" target="_blank"><img src="/static/img/pdf.png" alt="PDF"></a></td>
                    <td>Title: <a :href="element.file_path" target="_blank">{{element.title}}</a><br>
...
export default {
    ...
    created() {
        this.output = this.$route.query.array;
    },
    methods: {
        // display: function() {
        ...
        search: async function() {
            ...
            .then(response => {
                ...
                for(let i=0; i<this.hits.length; i++) {
                    ...
                    this.array[i].Did = this.hits[i].id;
                    this.array[i].position = this.array[i].data.indexOf(this.input);
                    console.log(this.array[i].data.substring(this.array[i].data.indexOf(this.input), this.array[i].data.indexOf(this.input)+20));
                }
                console.log(this.array);
